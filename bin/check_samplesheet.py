...
class RowChecker:
    ...
    VALID_ORGANISMS = {
        ...
        "klebsiella_pneumoniae_complex": "Klebsiella_pneumoniae_complex",
    ...
    def __init__(
        self,
        sample_col="sample",
        first_col="fastq_1", # values will have to be NA or '' if not being used
        second_col="fastq_2", # values will have to be NA or '' if not being used
        third_col='assembly', # values will have to be NA or '' if not being used
        fourth_col='organism',
        **kwargs,
    ):
        """
        ...
        Args:
            ...
            third_col (str): The name of the column that contains the assembly file paths, if not being used, values must be NA or ''
    ...
    def _validate_fastq_and_assembly(self, row):
        ...
        if not fastq_1 and not fastq_2:
            if self._third_col: #verify that the assembly column exists
                assembly = row.get(self._third_col, "")
                if len(assembly) <= 0:
    ...
    def _validate_organism(self, row):
        organism = row[self._fourth_col].strip()
        if not organism:
            raise AssertionError("Organism is required.")

        normalized = organism.replace(" ", "_").lower()

        if normalized not in self.VALID_ORGANISMS:
            raise AssertionError(
                f"Invalid organism '{organism}'. "
                f"Allowed values are: {', '.join(self.VALID_ORGANISMS.values())}"
            )

        row[self._fourth_col] = self.VALID_ORGANISMS[normalized]
...
def check_samplesheet(file_in, file_out):
    ...
    Example:
        ...
        see also the `viral recon samplesheet`_::
            ...
            SAMPLE_PE,SAMPLE_PE_RUN1_1.fastq.gz,SAMPLE_PE_RUN1_2.fastq.gz,SAMPLE_PE.fna,organism
            SAMPLE_PE,SAMPLE_PE_RUN2_1.fastq.gz,SAMPLE_PE_RUN2_2.fastq.gz,SAMPLE_PE.fna,organism
            SAMPLE_SE,SAMPLE_SE_RUN1_1.fastq.gz,SAMPLE_PE.fna,organism
            SAMPLE,,,,SAMPLE.fna,organism

    .. _viral recon samplesheet:
        https://raw.githubusercontent.com/nf-core/test-datasets/viralrecon/samplesheet/samplesheet_test_illumina_amplicon.csv

    """
    required_columns = ["sample", "fastq_1", "fastq_2", "assembly", "organism"]  #
